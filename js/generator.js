...
pontoEmbarque = "";
// Iniciando o mapa
var map = L.map('map', {zoomControl: false}).setView([-21.786, -46.566], 15);
...
Generator.drawAgencyStop = function (data) {
  ...
  L.marker([-23.543845, -46.631352], {icon: agencyMarker}).addTo(map)
...
// Desenha uma lista de opções de rotas
...
Generator.drawShapes = function (data) {
  ...
  for (var y = 0; y < obj[0].length; y++) {
    var marker = L.marker([obj[0][y].shape_pt_lat, obj[0][y].shape_pt_lon], {icon: greenIcon});
  ...
  var polyline = L.polyline(latlngs, {color: '#3498db'}).addTo(map);
...
function test(data){
  ...
  var url = "http://127.0.0.1:8124/api/stops?agency_key=SPTRANS&route_id=" + obj[0].route_id + "&direction_id=0";
  console.log(url);
  get(url, "stops");
}
